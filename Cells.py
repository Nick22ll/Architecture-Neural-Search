...
class NormalCell(nn.Module, dgl.DGLGraph):
    def __init__(self, edge_operations, input_dim, in_channels, channels, cell_nodes, device):
        ...
        for edge_idx in range(self.num_edges()):
            ...
            if self.edge_operations_dict[edge_idx]["operation"] in [3]:  # if convolution
                op = nn.Sequential(OPERATIONS[self.edge_operations_dict[edge_idx]["operation"]](channels, channels, self.edge_operations_dict[edge_idx].get("kernel_size", 3), stride=1, padding=math.floor(self.edge_operations_dict[edge_idx].get("kernel_size", 3) / 2), bias=False, device=device),
...
class ReductionCell(nn.Module, dgl.DGLGraph):
    def __init__(self, edge_operations, input_dim, in_channels, channels, cell_nodes, device):
        ...
        for edge_idx in range(self.num_edges()):
            ...
            if self.edge_operations_dict[edge_idx]["operation"] in [3]:  # if convolution
                op = nn.Sequential(OPERATIONS[self.edge_operations_dict[edge_idx]["operation"]](self.channels, self.channels, self.edge_operations_dict[edge_idx].get("kernel_size", 3), stride=stride, padding=math.floor(self.edge_operations_dict[edge_idx].get("kernel_size", 3) / 2), bias=False, device=device),
                                   nn.BatchNorm2d(channels, affine=False, device=device))
            elif self.edge_operations_dict[edge_idx]["operation"] in [1, 2]:
                op = nn.Sequential(OPERATIONS[self.edge_operations_dict[edge_idx]["operation"]](self.edge_operations_dict[edge_idx].get("kernel_size", 3), stride=stride, padding=math.floor(self.edge_operations_dict[edge_idx].get("kernel_size", 3) / 2)),
                                   nn.BatchNorm2d(self.channels, affine=False, device=device))
            elif self.edge_operations_dict[edge_idx]["operation"] in [0]:
                op = ZeroOp(stride)
            elif self.edge_operations_dict[edge_idx]["operation"] in [4]:
