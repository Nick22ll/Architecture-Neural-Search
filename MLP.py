...
class GenericMLP(nn.Module):
    def __init__(self, in_feats, hidden_dim, out_dim, dropout):
        ...
        ####  Normalization Layers  ####
        self.Norm1 = nn.BatchNorm1d(hidden_dim, eps=1e-05, momentum=0.1, affine=False)
        self.Norm2 = nn.BatchNorm1d(hidden_dim, eps=1e-05, momentum=0.1, affine=False)
    ...
    def forward(self, feats):
        ...
        updated_feats = self.Norm1(updated_feats)
        updated_feats = self.LeakyReLU(updated_feats)

        # SECOND LAYER
        updated_feats = self.hidden_linear(updated_feats)
        updated_feats = self.Norm2(updated_feats)
        updated_feats = self.LeakyReLU(updated_feats)
